...
import styles from './CreateOrders.module.css' 
        ...
        <main className={styles.create}>
            <div className={styles.menu}>
                ...
                <div className={styles.filter}>
                    {filters.map((filter) => (
                        <Button type="button" color={(!searchParams.get('category') && filter === 'All' ||
                    ...
                    {menus.map((item : IMenu) => (
                        <div className={styles.item} key={item.id}>
                            ...
                            <div className={styles.bottom}>
                                ...
                                <Button onClick={()=> handleAddToCart('increment',item.id,item.name)}>Order</Button>
                            </div>
                        </div>
                            ...
                            {carts.map((item : ICart) =>(
                                <div className={styles.item} key={item.menuItemId}>
                                    ...
                                    <div className={styles.quantity}>
                                        ...
                                        <Button className={styles.button} onClick={()=> handleAddToCart('decrement',`${item.menuItemId}`,`${item.name}`)} color="secondary">
                                            -
                                        </Button>
                                        <div className={styles.number}>{item.quantity}</div>
                                        <Button className={styles.button} onClick={() => handleAddToCart('increment',`${item.menuItemId}`,`${item.name}`)} color="secondary">
                                            +
                                        </Button>
                                    </div>
                                </div>
